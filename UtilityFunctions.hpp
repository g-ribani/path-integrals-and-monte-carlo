...
std::vector<double>
   LinearRange(double in, double fin, std::vector<double>::size_type n) {
      std::vector<double> ret;
      if(n == 1) ret.push_back((in + fin)/2.); // mid point if n == 1
      else if(n > 1) {
         double step = (fin - in)/(n - 1);
         for(std::vector<double>::size_type k = 0; k != n; ++k)
